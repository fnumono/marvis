...
exports.call = function(agenda, event, task, number) {
  agenda.define(task, function(job, done) {
    ...
    client.makeCall({
      ...
      url: 'http://3c1ec152.ngrok.io/call/?number=14084253396' + event.number + '&eventName=' + encodeURIComponent(event.eventName) // A URL that produces an XML document (TwiML) which contains instructions for the call
